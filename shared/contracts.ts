...
// ============================================
// BID PERIOD BASELINE — Awarded credit tracking
// Defined here (before dashboard) because dashboard embeds baseline analysis.
// ============================================

// Source of the baseline value
export const bidPeriodBaselineSourceValues = [
  "uploaded_award",
  "manual_entry",
  "estimated",
] as const;
export type BidPeriodBaselineSource = (typeof bidPeriodBaselineSourceValues)[number];

// Confidence of the baseline value
export const bidPeriodBaselineConfidenceValues = [
  "high",
  "medium",
  "low",
] as const;
export type BidPeriodBaselineConfidence = (typeof bidPeriodBaselineConfidenceValues)[number];

// The stored baseline record for one period
export const bidPeriodBaselineSchema = z.object({
  id: z.string(),
  periodKey: z.string(),          // e.g. "2026-P03"
  periodStartISO: z.string(),
  periodEndISO: z.string(),
  awardedCreditMinutes: z.number(),
  source: z.enum(bidPeriodBaselineSourceValues),
  sourceNote: z.string().nullable(),
  uploadId: z.string().nullable(),
  confidence: z.enum(bidPeriodBaselineConfidenceValues),
  createdAt: z.string(),
  updatedAt: z.string(),
});
export type BidPeriodBaseline = z.infer<typeof bidPeriodBaselineSchema>;

// Dashboard baseline analysis — embedded in GetDashboardResponse
export const baselinePayAnalysisSchema = z.object({
  awardedCreditMinutes: z.number(),
...
export type BaselinePayAnalysis = z.infer<typeof baselinePayAnalysisSchema>;
...
export const getDashboardResponseSchema = z.object({
  ...
  // Awarded baseline analysis (present when baseline is available)
  baselineAnalysis: baselinePayAnalysisSchema.optional(),
});
...
export const tripSchema = z.object({
  ...
  protectedCreditMinutes: z.number().optional(),
  currentCreditMinutes: z.number().optional(),
  payCreditMinutes: z.number().optional(),
  // Trip action / pickup classification
  tripActionType: z.string().optional(),
...
export const flightRegisterDataSchema = z.object({
  payPeriodStart: z.string().nullable(),
  payPeriodEnd: z.string().nullable(),
  beginningPayCredit: z.string().nullable(),
  endingPayCredit: z.string().nullable(),
  dutyDays: z.number().nullable(),
  jaHours: z.string().nullable(),
  ja2Hours: z.string().nullable(),
  blockHoursPaid: z.string().nullable(),
  tdyPerDiem: z.number().nullable(),
  // v4 — pay bucket fields parsed from FR totals section
  overGuaranteeAmountDollars: z.number().nullable().optional(),
  underGuaranteeAmountDollars: z.number().nullable().optional(),
  premiumPayAmountDollars: z.number().nullable().optional(),
  taxablePdmAmountDollars: z.number().nullable().optional(),
  jaAmountDollars: z.number().nullable().optional(), // JA dollar amount from FR (exact, avoids recomputed rounding)
});
export type FlightRegisterData = z.infer<typeof flightRegisterDataSchema>;

export const dayforceDataSchema = z.object({
  payPeriodStart: z.string().nullable(),
  payPeriodEnd: z.string().nullable(),
  payRate: z.number().nullable(),
  advNextPay: z.number().nullable(),
  overGuaranteeHours: z.number().nullable(),
  overGuaranteeAmount: z.number().nullable(),
  underGuarantee: z.number().nullable(),
  juniorHours: z.number().nullable(),
  juniorAmount: z.number().nullable(),
  premiumPayHours: z.number().nullable(),
  premiumPayAmount: z.number().nullable(),
  domicilePdmTx: z.number().nullable(),
  dmsticPdmTx: z.number().nullable(),
  vacationHours: z.number().nullable(),
  vacationAmount: z.number().nullable(),
  grossPay: z.number().nullable(),
  netPay: z.number().nullable(),
});
export type DayforceData = z.infer<typeof dayforceDataSchema>;

export const payAuditComparisonSchema = z.object({
  // JA fields are nullable — null means UNKNOWN (not zero)
  jaHourDifference: z.number().nullable(),
  jaDollarDifference: z.number().nullable(),
  jaExpectedPay: z.number().nullable(),
  jaActualPay: z.number().nullable(),
  overGuaranteeDifference: z.number(),
  grossPayDifference: z.number().nullable(),
  baseGuaranteeExpected: z.number(),
  // Data quality metadata
  jaStatus: z.enum(['found', 'not_found', 'low_confidence']).optional(),
  periodsMatch: z.boolean().optional(),
});
export type PayAuditComparison = z.infer<typeof payAuditComparisonSchema>;
...
export const payAuditResultSchema = z.object({
  id: z.string().optional(),
  matchScore: z.number(),
  matchStatus: z.enum(['paid_correctly', 'mostly_matched', 'possible_discrepancy', 'likely_issue', 'mismatched_periods', 'unable_to_verify']),
  estimatedDifference: z.number(),
  summary: z.string(),
  findings: z.array(z.string()),
  flightRegister: flightRegisterDataSchema,
  dayforce: dayforceDataSchema,
  appData: z.object({
    payRate: z.number(),
    position: z.string().nullable(),
    base: z.string().nullable(),
  }),
  comparison: payAuditComparisonSchema,
  createdAt: z.string().optional(),
  // Flight Register-only audit fields (v3)
  expectedGrossCents: z.number().optional(),
  estimatedNetCents: z.number().optional(),
  enteredSettlementCents: z.number().optional(),
  enteredAdvanceCents: z.number().optional(),
  comparisonMode: z.enum(['gross', 'net']).optional(),
  auditDifferenceCents: z.number().optional(),
  auditStatus: z.enum(['paid_correctly', 'minor_variance', 'review_recommended', 'possible_discrepancy', 'likely_issue']).optional(),
  auditSummary: z.string().optional(),
  // v4 — matched Dayforce period guidance + gross breakdown buckets
  matchedSettlementPeriod: z.string().nullable().optional(),
  matchedAdvancePeriod: z.string().nullable().optional(),
  matchedSettlementPayDate: z.string().nullable().optional(),
  matchedAdvancePayDate: z.string().nullable().optional(),
  expectedGrossBreakdown: z.object({
    guaranteeCents: z.number(),
    jaCents: z.number(),
    premiumPayCents: z.number(),
    overUnderCents: z.number(),
    taxablePdmCents: z.number(),
  }).nullable().optional(),
});
export type PayAuditResult = z.infer<typeof payAuditResultSchema>;

export const payAuditAnalyzeRequestSchema = z.object({
  flightRegisterImages: z.array(z.string()),
  dayforceImages: z.array(z.string()),
  mimeType: z.string().optional(),
});
export type PayAuditAnalyzeRequest = z.infer<typeof payAuditAnalyzeRequestSchema>;

export const payAuditListItemSchema = z.object({
  id: z.string(),
  createdAt: z.string(),
  matchScore: z.number().nullable(),
  matchStatus: z.string().nullable(),
  estimatedDifference: z.number().nullable(),
  payPeriodStart: z.string().nullable(),
  payPeriodEnd: z.string().nullable(),
  summary: z.string().nullable(),
});
export type PayAuditListItem = z.infer<typeof payAuditListItemSchema>;

// JA (Junior Assignment) pay multiplier
export const JA_PAY_MULTIPLIER = 1.5;

// ============================================
// BID PERIOD BASELINE API contracts
// (schema types defined before dashboard above)
// ============================================

// GET /api/bid-period-baseline?periodKey=2026-P03
export const getBidPeriodBaselineResponseSchema = z.object({
  baseline: bidPeriodBaselineSchema.nullable(),
  periodKey: z.string(),
  periodStartISO: z.string(),
  periodEndISO: z.string(),
  hasImportedTrips: z.boolean(),
  currentCreditMinutes: z.number(),
  guaranteeMinutes: z.number(),
});
export type GetBidPeriodBaselineResponse = z.infer<typeof getBidPeriodBaselineResponseSchema>;

// PUT /api/bid-period-baseline — set or update baseline
export const upsertBidPeriodBaselineRequestSchema = z.object({
  periodKey: z.string(),
  awardedCreditHours: z.number().min(0).max(200),
  source: z.enum(bidPeriodBaselineSourceValues),
  sourceNote: z.string().optional(),
  uploadId: z.string().optional(),
  confidence: z.enum(bidPeriodBaselineConfidenceValues).optional(),
});
export type UpsertBidPeriodBaselineRequest = z.infer<typeof upsertBidPeriodBaselineRequestSchema>;

export const upsertBidPeriodBaselineResponseSchema = z.object({
  success: z.boolean(),
  baseline: bidPeriodBaselineSchema,
});
export type UpsertBidPeriodBaselineResponse = z.infer<typeof upsertBidPeriodBaselineResponseSchema>;

// DELETE /api/bid-period-baseline/:periodKey — clear baseline for a period
export const deleteBidPeriodBaselineResponseSchema = z.object({
  success: z.boolean(),
});
export type DeleteBidPeriodBaselineResponse = z.infer<typeof deleteBidPeriodBaselineResponseSchema>;
