...
const NAV_ITEMS = [
  ...
  { to: '/pay-summary', label: 'Pay Summary', icon: DollarSign },
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        cn(
          ...
          isActive
            ? 'bg-blue-600/15 text-blue-400 sidebar-active'
...
export default function AppLayout() {
  ...
  const currentPageTitle = PAGE_META[location.pathname] ?? 'PilotPay';
  ...
  return (
    <div className="app-root flex bg-navy-950">
      {/* ── Desktop Sidebar (lg+): in-flow, collapsible ── */}
      <aside
        className={cn(
      ...
      {/* ── Mobile/Tablet Sidebar (<lg): fixed overlay ── */}
      {/* Backdrop */}
      <div
        className={cn(
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden app-main">
        ...
        <header className="flex-shrink-0 h-14 border-b border-white/[0.05] flex items-center px-4 lg:px-6 gap-3 bg-[#080d19]/80 backdrop-blur-sm">
          ...
          <button
            ...
            className="lg:hidden p-2 -ml-1 rounded-lg text-slate-400 hover:text-white hover:bg-white/[0.06] active:bg-white/[0.08] transition-colors touch-manipulation"
